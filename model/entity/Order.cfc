		...
		if(!structKeyExists(variables, "deliveredItemsAmountTotal")) {
			...
			for(var orderItem in getOrderItems()) {
				...
				if(orderItem.getQuantityDelivered()) {
					...
					vvariables.deliveredItemsAmountTotal = precisionEvaluate(variables.deliveredItemsAmountTotal + ((orderItem.getQuantityDelivered() / orderItem.getQuantity()) * orderItem.getItemTotal()));
